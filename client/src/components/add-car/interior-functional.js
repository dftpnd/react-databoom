...
class InteriorFunctional extends React.Component {
	constructor(props) {
		...
		const defaultState = {
			ifPedals: true,
			ifInteriorLighting: true,
			ifPowerWindows: true,
			ifCentralLocking: true,
			ifMirrorSettings: true,
			ifSwitchGear: true,
			ifParktronic: true,
			ifPowerConnectors: true,
			ifAirConditioning: true,
			ifSoundSystem: true,
			ifNavigationSystem: true,
			ifMicrophone: true,
			ifSteeringWheel: true,
			ifInstrumentPanelLights: true,
			ifWiper: true,
			ifHeadlightWasher: true,
			ifSideWindows: true,
			ifGloveLighting: true,
			ifSteeringAdjustment: true,
			ifSeatAdjustment: true,
			ifAdjustment: true,
			ifAirbagsErrors: true,
			ifBoardComputer: true
		};
	...
	render() {
		return (
			<div className="interior-functional">
				<h1 className="app-title">Функционал Салона</h1>
